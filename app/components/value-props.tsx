      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {props.map((prop, i) => (
            <div
              key={prop.label}
              className="reveal group rounded-xl border border-off-white/5 bg-surface-mid/30 backdrop-blur-sm p-6 transition-all duration-300 hover:border-copper/20 hover:bg-surface-mid/50"
              style={{ transitionDelay: `${i * 100}ms` }}
            >
              <p className="text-2xl font-bold text-copper-light">{prop.stat}</p>
